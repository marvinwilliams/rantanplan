...
void Predicate::add_parameter_type(Handle<Type> type) {
  if (problem_ != type.get_problem()) {
    throw ModelException{"Type is not from the same problem"};
  }
  parameter_types_.push_back(type.get());
}
...
Handle<Parameter> Action::add_parameter(std::string name, Handle<Type> type) {
  ...
  return {parameters_.back().get(), this, problem_};
}

void Action::set_precondition(std::unique_ptr<Precondition> precondition) {
  if (problem_ != precondition->get_problem() ||
...
void Action::set_effect(std::unique_ptr<Effect> effect) {
  if (problem_ != effect->get_problem() || this != effect->get_action()) {
...
void Problem::add_parameter_type(Handle<Predicate> predicate,
  ...
  predicate.p_->add_parameter_type(type);
}
...
void Problem::add_init(std::unique_ptr<InitPredicate> init) {
