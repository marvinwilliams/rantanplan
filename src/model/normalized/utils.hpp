...
inline bool update_condition(Condition &condition, const Action &action) {
  bool ground = true;
  for (Argument &a : condition.atom.arguments) {
    if (a.is_parameter()) {
      if (!action.parameters[a.get_parameter_index()].is_free()) {
...
inline size_t get_num_instantiated(const Predicate &predicate,
  ...
  return std::accumulate(
      ...
      [&problem](size_t product, TypeIndex type) {
        return product * problem.constants_of_type[type].size();
      });
}
...
inline size_t get_num_instantiated(const Action &action,
  ...
  return std::accumulate(
      ...
      [&problem](size_t product, const Parameter &p) {
        ...
        return product * problem.constants_of_type[p.get_type()].size();
      });
}
...
inline size_t get_num_instantiated(const ParameterSelection &selection,
  ...
  return std::accumulate(
      ...
      [&action, &problem](size_t product, ParameterIndex index) {
        auto type_index = action.parameters[index].get_type();
...
class AssignmentIterator {
  ...
  void set_assignment() {
    ...
    for (size_t i = 0; i < combination.size(); ++i) {
      ...
      auto type_index = action_->parameters[(*selection_)[i]].get_type();
  ...
  explicit AssignmentIterator(const ParameterSelection &selection,
    ...
    for (auto p : selection) {
      ...
      argument_size_list.push_back(
          problem.constants_of_type[action.parameters[p].get_type()].size());
    }
...
public:
  ...
  explicit GroundAtomIterator(const Atom &atom, const Action &action,
      ...
      : ground_atom_{atom.predicate,
                     std::vector<ConstantIndex>(atom.arguments.size())},
...
inline bool is_instantiatable(const Atom &atom,
  ...
  if (is_ground(atom)) {
    for (size_t i = 0; i < atom.arguments.size(); ++i) {
      if (atom.arguments[i].get_constant() != arguments[i]) {
        return false;
      }
    }
    return true;
  ...
  for (size_t i = 0; i < atom.arguments.size(); ++i) {
    if (!atom.arguments[i].is_parameter()) {
      ...
      if (!p.is_free()) {
        ...
        if (!is_subtype(problem.constants[arguments[i]].type, p.get_type(),
                        problem)) {
          return false;
        }
        p.set(arguments[i]);
  ...
  for (size_t i = 0; i < first_atom.arguments.size(); ++i) {
    ...
    if (!first_p.is_parameter() && !second_p.is_parameter()) {
      ...
      if (!action_p.is_free()) {
        ...
        if (!is_subtype(problem.constants[second_p.get_constant()].type,
      ...
      if (!action_p.is_free()) {
        ...
        if (!is_subtype(problem.constants[first_p.get_constant()].type,
                        action_p.get_type(), problem)) {
          return false;
        }
...
template <> struct hash<normalized::ParameterAssignment> {
  size_t operator()(const normalized::ParameterAssignment &assignment) const
    ...
    for (const auto &[parameter_index, constant] : assignment) {
      ...
      h ^= hash<normalized::ConstantIndex>{}(constant);
    }
    return h;
  }
};
